...
### FabAds shell — 2 nav variants (as of iter-6 A-2, 2026-05-01)

`rail` (default) / `sections`. Switched via the single-icon `NavVariantToggle`
in the bottom dock (mirrors dark/light toggle pattern — one click cycles).
Hook: `src/components/sidebar/useFabAdsNavVariant.ts`. Both variants share
the MODULES source of truth at `src/components/sidebar/modules.ts`.

When designing a new shell-level component (anything in the sidebar, header,
or footer dock), test it in both Rail and Sections variants before shipping.
The Sections variant adds functional group labels (RUN / DISCOVER / CREATE /
AUTOMATE) that components must be aware of.
