...
/**
 * Studio variant — Progress screen.
 * Clean centered hero + linear progress bar + stage pipeline + preview tiles.
 */
export function StudioProgressScreen() {
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-center gap-12 px-6 py-16">
      <div className="g6-fade-up relative flex flex-col items-center gap-6">
        <PulsingRingLoader size={160} />
        <div className="space-y-1.5 text-center">
          <p className="text-g6-xs uppercase tracking-wider text-g6-text-tertiary font-medium">
      ...
      <div className="w-full max-w-xl space-y-3">
        ...
        <div className="flex items-center justify-between gap-1">
          {STAGES.map((s, i) => {
            ...
            return (
              <div key={s.id} className={cn("flex flex-col items-center gap-1 transition-opacity", !done && !active && "opacity-40")}>
                <span className={cn(
      ...
      {previews.length > 0 && (
        <div className="g6-fade-up flex w-full max-w-xl flex-col gap-3">
          <p className="text-g6-xs uppercase tracking-wider text-g6-text-tertiary text-center">
            preview · {previews.length} of {Math.max(4, previews.length + 1)} ready
          </p>
          <div className="flex justify-center gap-3">
            {previews.map((output, idx) => (
              <div key={output.id} className="g6-fade-up h-28 w-24 overflow-hidden rounded-g6-card bg-g6-bg-container shadow-g6-sm" style={{ animationDelay: `${idx * 80}ms` }}>
                {output.thumbnail ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-g6-bg-spotlight">
                    <span className="text-g6-xs text-g6-text-tertiary">TXT</span>
                  </div>
                )}
              </div>
            ))}
            <div className="h-28 w-24 animate-pulse rounded-g6-card bg-g6-bg-spotlight" />
          </div>
        </div>
      )}
      ...
      <button
        ...
        className="rounded-g6-pill border border-g6-border-secondary bg-g6-bg-container px-5 py-2 text-g6-sm font-medium text-g6-text-secondary transition-colors hover:text-g6-text"
      >
        Cancel generation
      </button>
    </div>
