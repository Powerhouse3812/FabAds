/**
 * Creative Report 2.0 — automated-workflows sync-history store.
 *
 * Rules auto-fire and "sync" creatives to Meta ad-account libraries (a
 * simulated background upload — see syncModel.ts for `simulated: true`).
 * This store is the single source of truth for four consumer surfaces
 * (creative card badge, board detail, rule detail, bulk-sync modal) plus
 * the duplicate-upload guard in `enqueueSync`.
 *
...
import { pairKey, syncDurationMs, type SyncRecord } from "@/creative-report-v2/automations/sync/syncModel";
import { ACCOUNT_BY_ID } from "@/data/accounts";

export interface SyncStoreState {
  records: Record<string, SyncRecord>;
  /** ruleId → creativeIds the rule has currently acted on (edge-trigger marks). */
  firedByRule: Record<string, string[]>;
  lastPassAt: string | null;
}

const KEY = "creative-report-v2-sync-history";
...
const DEFAULT_STATE: SyncStoreState = { records: {}, firedByRule: {}, lastPassAt: null };
...
function sanitize(raw: unknown): SyncStoreState {
  ...
  const { records, firedByRule, lastPassAt } = raw as Partial<SyncStoreState>;
  ...
  const validFiredByRule: Record<string, string[]> = {};
...
/** Dev/demo affordance — wipes all sync history and fired-rule marks. */
