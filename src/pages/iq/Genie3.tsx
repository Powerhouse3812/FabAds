...
import { Genie3Wizard } from "@/components/genie3/Genie3Wizard";
...
export default function Genie3() {
  ...
  const handleRetry = useCallback((gen: GenieGeneration) => {
    batchGenerate({
      prompt: gen.prompt, settings: (gen.settings || {}) as GenieSettings,
  ...
  const handleClearFailed = useCallback(async () => {
    ...
    for (const g of failedGens) await deleteMutation.mutateAsync(g.id);
            ...
            <ScrollArea className="flex-1">
              <div className="p-4 2xl:p-5 space-y-6 max-w-4xl mx-auto">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">AI-powered creative workspace — guided flow</p>
                </div>
